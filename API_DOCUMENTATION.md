...
#### [Customer Import API →](./CUSTOMER_IMPORT_API.md)

Push transactions, monthly accounts, and parking subscriptions from your own systems (PMS, POS, custom backends) into Dash Marketing in batched JSON. Built for nightly syncs, backfills, and near-real-time forwarders.

**Key Features:**
- Three batch endpoints: transactions, accounts, subscriptions
- Stable per-record idempotency keys — replay-safe
- Resolve or create related entities (account, contact, vehicle, subscription) inline with each transaction
- Per-record outcomes (`Created` / `Updated` / `Skipped` / `Failed`) with structured reason codes
- Companion endpoints to look up your location UUIDs and replay prior import results

**Postman collection:** [dash-customer-import.postman_collection.json](./dash-customer-import.postman_collection.json) — import into Postman, set your `apiKey`, and run the folders top-to-bottom.
    ...
    "transactions": [
      {
        ...
        "convenienceFee": 0.85,
        "processingFee": 0.25,
        "contact": { "email": "driver@example.com" },
        "vehicle": { "licensePlate": "ABC1234", "state": "NY" }
      }
    ]
  }'
```

[View Full Documentation →](./CUSTOMER_IMPORT_API.md)
